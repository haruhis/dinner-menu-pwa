...
以下のJSONフォーマット（配列形式）で, 合計10件のレシピを提案してください。
...
ユーザーが食事の味付けに飽きないよう、直近と重複しない異なる味付けジャンル（例：ケチャップ・トマト味、カレー・スパイシー系、洋風クリーム、酸味の効いたさっぱり塩レモン、中華風ピリ辛など）のバリエーション豊かなメニューを優先的に組み立てて提案してください。` : `バリエーション豊かな味付け（醤油ベースの甘辛、塩味、味噌、トマトケチャップ、カレーなど）のメニューをバランスよく提案してください。`}

提案の条件：
1. **【最重要】提案する10件のうち、必ず3件は「【おすすめセット】（主菜・副菜・スープ等の汁物を組み合わせた1食丸ごとのMeal・セットメニュー）」として提案してください。**
...
3. 全体の中で、5件程度は「今ある材料で作れるもの」、残り5件程度は「あと1つだけ特定の食材を買い足せば作れるもの（要買い足し）」のバランスにしてください（おすすめセットがどちらに該当しても構いません）。
