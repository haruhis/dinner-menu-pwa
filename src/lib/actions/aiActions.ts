...
- 余計な説明テキストや、トリプルバッククォート（\`\`\`json）などのマークダウンの囲み記号は一切出力せず、JSON.parseでそのまま処理できる純粋なJSON配列のみを返してください。`;
  ...
  const finalSystemPrompt = systemPrompt
    ...
    .replace(/異なる「スタイル役割」の義務化（改善案1）/g, "味わいや調理アプローチの自由なバリエーション")
    .replace(/提案する3〜4件のレシピには、マンネリ化を防ぐため、以下の4つのうちから重複しない異なるスタイル役割を必ず各レシピに1つずつ割り当てて設計してください。/g, "提案するレシピには、特定の決まったパターン枠に囚われる必要は一切ありません。その食材に最も適した、味わい（塩、甘辛醤油、トマト、カレー、ピリ辛中華など）や、調理アプローチ（定番、スピード時短、一工夫創作など）に豊かなバリエーションを持たせて自由に設計してください。")
    .replace(/含めても構いません。/g, "必ずタイトルの冒頭に【AIが決定した料理の最大の魅力や特徴を表すタグ】（例：【定番の甘辛】、【さっぱり時短】、【旨辛エスニック】、【コク旨創作】、【ほっこり洋風】など）を付与して開始してください。これは画面バッジ表示のために【絶対遵守】の出力規則です。")
    .replace(/含めても構いません/g, "必ずタイトルの冒頭に【AIが決定した魅力タグ】を付与して開始してください（絶対遵守）")
  ...
  const contents = [
    {
      role: "user",
      parts: [{ text: finalSystemPrompt.replace(/夕食の献立を提案してください。/g, "夕食（晩ごはん・夜ご飯）の献立を提案してください。なお、提案するすべてのレシピは「夕食（晩ごはん・夜ご飯・ディナー）」であることを大前提とします。タイトルの表現やレシピの説明において、「ランチ」「お昼ご飯」「お弁当」「朝食」などの他時間帯を連想させる単語は【絶対に】使用しないでください。今日の夜に食べて大満足する夕食メニューとして組み立てて出力してください。") }]
    }
  ];
