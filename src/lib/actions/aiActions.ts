...
- 余計な説明テキストや、トリプルバッククォート（\`\`\`json）などのマークダウンの囲み記号は一切出力せず、JSON.parseでそのまま処理できる純粋なJSON配列のみを返してください。`;
  ...
  const finalSystemPrompt = systemPrompt
    ...
    .replace(/含めても構いません。/g, "必ずタイトルの冒頭に【スタイル役割名】（例：【おしゃれなカフェ・モダン風】ハニーマスタードチキン）を付与して開始してください。これは画面バッジ表示のために【絶対遵守】の出力規則です。")
    .replace(/含めても構いません/g, "必ずタイトルの冒頭に【スタイル役割名】を付与して開始してください（絶対遵守）")
