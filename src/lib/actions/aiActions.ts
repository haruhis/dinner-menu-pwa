...
1. **食材の重複回避と主材料の多様性（最重要）**
   提案する4つのレシピ（おすすめセット1件 ＋ 単品3件）は、それぞれ異なる主材料をメインに据えてください（例：1つ目が鶏肉なら、2つ目は魚、3つ目はナス、4つ目は豆腐や豚肉など）。4つのレシピすべて、または大部分が特定の1つの食材（例：ズッキーニのみ、豚肉のみなど）に偏ることは絶対に避けてください。
2. **味わいや調理アプローチの自由なバリエーション**
   提案するレシピには、特定の決まったパターン枠に囚われる必要は一切ありません。その食材に最も適した、味わい（塩、甘辛醤油、トマト、カレー、ピリ辛中華など）や、調理アプローチ（定番、スピード時短、一工夫創作など）に豊かなバリエーションを持たせて自由に設計してください。
   必ずタイトルの冒頭に【AIが決定した料理の最大の魅力や特徴を表すタグ】（例：【定番の甘辛】、【さっぱり時短】、【旨辛エスニック】、【コク旨創作】、【ほっこり洋風】など）を付与して開始してください。これは画面バッジ表示のために【絶対遵守】の出力規則です。
...
3. **身近な調味料の新鮮な掛け合わせ**
   ...
   - 【毎日使うアプリとしての重要事項（アジアン・エスニック調味料の不採用）】ナンプラー、コチュジャン、スイートチリソースなどのアジアン・エスニック風の特殊な調味料、または入手しにくいハーブ類・スパイス類は**提案に含めないでください**。毎日使うアプリとして、身近な調味料の範囲で新鮮さを出す実用的な提案に限定してください。
...
提案の条件：
1. **【最重要】提案する4件のうち、必ず1件は「【おすすめセット】（主菜・副菜・スープ等の汁物を組み合わせた1食丸ごとのMeal・セットメニュー）」として提案してください。**
   - タイトル（"title"）は「【おすすめセット】〇〇と〇〇の〜〜セット」（買い出しが必要な場合はさらに「（要: 〇〇追加）」を付与）のように記載してください。和風・洋風（例: パスタ＋スープ＋サラダ）・中華風をバランスよく出力すること。
...
2. 残りの3件は、従来どおりの単品おかずとして提案してください。
3. 全体の中で、2件程度は「今ある材料で作れるもの」、残り2件程度は「あと1つだけ特定の食材を買い足せば作れるもの（要買い足し）」のバランスにしてください（おすすめセットがどちらに該当しても構いません）。

【絶対遵守：夕食（晩ごはん・夜ご飯）コンセプトへの完全統一ルール】
提案するすべてのレシピは「夕食（晩ごはん・夜ご飯・ディナー）」であることを大前提とします。タイトルの表現やレシピの説明において、「ランチ」「お昼ご飯」「お弁当」「朝食」などの他時間帯を連想させる単語は【絶対に】使用しないでください。今日の夜に食べて大満足する夕食メニューとして組み立てて出力してください。

出力フォーマット（JSON配列）の各オブジェクトのスキーマ：
[
  {
    "title": "料理名（今ある材料で作れる場合はそのまま、買い出しが必要な場合は『料理名（要: ○○追加）』のようにタイトルに明記。おすすめセットの場合は必ず『【おすすめセット】』から開始し、単品の場合は必ず『【味わい魅力タグ】』から開始すること）",
    "description": "料理の説明（食欲をそそる魅力的な紹介文、買い足しレシピの場合は『※「○○」を買い足すことで...』という説明を必ず含める。どのような『スタイル役割』や『調味料の掛け合わせ』の工夫があるかも魅力的に紹介し、おすすめセットの場合は主菜・副菜・汁物の内訳を明記すること）",
...
- 余計な説明テキストや、トリプルバッククォート（\`\`\`json）などのマークダウンの囲み記号は一切出力せず、JSON.parseでそのまま処理できる純粋なJSON配列のみを返してください。`;
  ...
  try {
    const rawJson = await callGemini(contents, GEMINI_API_URL_LITE, {
