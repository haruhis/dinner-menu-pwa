...
export const aiService = {
  ...
  suggestMenus: async (params: SuggestionParams, trend?: DietaryAnalysis): Promise<MenuSuggestion[]> => {
    ...
    for (const recipe of RECIPE_CATALOG) {
      ...
      } else if (totalRequired === 1 && matchCount === 0) {
        // Single required item that user doesn't have, but we can treat the required item as the missing one
        const missingReq = recipe.required[0];
        missingOne.push({
          title: `${recipe.title} (要: ${missingReq}追加)`,
          description: `${recipe.description} ※「${missingReq}」を1つ買い足せば作ることができます。`,
          missingIngredient: missingReq,
          steps: recipe.steps
        });
      }
